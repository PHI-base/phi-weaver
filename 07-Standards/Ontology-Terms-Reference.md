...
`phipo_extensions.tsv` by `phiweaver/lookup/extension_config.py` (source: public
PHI-base/canto-config — see `phiweaver/lookup/data/README.md`). Do **not** invent relations: use
only the attested set below. Run `python3 -m phiweaver.lookup.extension_config` to print the live
list, or `... infective_ability=PHIPO:0000015` to check one pair.
...
| `host_susceptibility` | PHIPO term under `PHIPO:0001456` | how prone the host is to infection/colonization/disease following exposure. Added upstream 2026-08-05; `PHIPO:0001456` itself is not yet in the vendored PHIPO release (see `phiweaver/lookup/data/README.md`), so a value there validates as attested but the term ID currently reads `not_found`. |

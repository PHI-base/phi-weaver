...
**Canonical for:** the lessons ledger — what was learned, and where each lesson was folded in.
The mechanism behind it is [`LEARNING-SYSTEM.md`](LEARNING-SYSTEM.md).
...
- `paper-review` — a curator's review of a specific paper's draft (e.g. Hsin-Yu on PMID:42089373);
...
| --- | --- | --- | --- | --- | --- | --- |
| L1 | 2026-07-15 | note | weaver draft; surfaced curating PMID:42089373 (to confirm w/ Hsin-Yu) | A phenotype that co-occurs with a **severe growth/fitness defect** must be annotated but either justified as growth-independent (normal growth / biomass-normalised readout / complementation rescue) or flagged as possibly pleiotropic; assert **no GO function** from a growth-confounded phenotype alone. | Conventions doc "Phenotype interpretation" section + `curation-qc` step-7 check | applied (pending curator confirmation) |
...
| L9 | 2026-07-20 | issue | jseager7 on PHI-base/phipo PR #454 → #455 | **When a missed concept already has a semantically-correct *obsolete* term, restore that term — do not mint a new duplicate ID.** OBO term-stability ([fp-019](https://obofoundry.org/principles/fp-019-term-stability.html)) requires reusing the original ID; re-creating it silently forks the concept. A term obsoleted only for **non-use** (not for being wrong) is a restore candidate — confirm with the parallel-terms test (L7/step 5) that the concept is genuinely live. | **`ontology-term-request` skill step 5** — the "missed" branch now forks on whether a correct obsolete term exists (restore vs. create), and the `PHIPO:0000503` worked example is corrected (#454 minted `PHIPO:0001456` in error; PR #455 restores `0000503`). | applied — **jseager7 ruling on the PR (#454 closed not-planned); PR #455 open for the restore.** |

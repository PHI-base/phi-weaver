...
| L6 | 2026-07-15 | paper-review | Hsin-Yun Chang, PMID:42089373 | **Conditions must use the controlled vocabulary (PHI-ECO, prefix `PECO:`):** weaver's free-text conditions (media/temp/duration) won't pass final approval. Treat conditions as advisory until PHI-ECO is vendored (mirror the PHIDO fix). | `validate_ontology_ids` now validates `PECO:` offline (vendored `phi-eco.obo`); reference in `07-Standards/Ontology-Terms-Reference.md` | applied (validation) — infra done; drafting still emits free-text conditions (surfacing the PECO vocab to drafting is the remaining step) |

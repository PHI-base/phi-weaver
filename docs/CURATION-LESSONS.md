...
| L8 | 2026-07-17 | curator ruling | Martin, closing the "complete loss of conidiation" backlog gap; verified against the bundled `phipo-base.obo` | **A phenotype phrase can miss PHIPO twice over on wording alone — and both misses are systematic, not bad luck.** PHIPO is **species-neutral by design** (its own header: `dc:description "Ontology of species-neutral phenotypes observed in pathogen-host interactions."`) because a term must hold across pathogens — *conidiation* exists only in some fungi, *asexual spores* in most organisms. So **(1) species-specific → species-neutral**: `conidiation`, `conidia`, `appressorium`, `haustorium`, `sclerotium`, `mycelium`, `ascospore`, `urediniospore` have **zero** primary-label hits; PHIPO says `asexual spores`, `sexual spores`, `hyphae`, `pathogen penetration structure`. And **(2) process noun → entity noun, for presence/absence**: both forms exist but carry **different dimensions** — the **process** (`asexual sporulation`) carries *quality* (`normal`/`abnormal`) and *timing* (`delayed`/`premature`) and has **no free-living absence term at all**; the **entity** (`asexual spores`) carries *count*, *size* and *presence/absence* (`asexual spores absent`/`present`). So "sporulation abolished" misses because absence is not a dimension of the process form. **Curator ruling: the entity being absent covers the process having failed — they are not distinct phenotypes.** The species vocabulary is *not* missing: it lives in **EXACT synonyms** (35 `conidi*`), so `map_phenotype "conidia absent"` → ★ `PHIPO:0000061`. Only the process noun `conidiation` is absent outright, so "conidiation completely lost" is an honest `no_match`. **This is L2's shape, systematised — and it had already produced a false gap:** "Complete loss of conidiation (free-living)" sat in BACKLOG as a real coverage gap when PHIPO:0000061 (free-living, `single_species_phenotype`) was exactly it. | `ontology-term-request` skill "not a gap" #1 (the two systematic retries, before any other); `docs/BACKLOG.md` gap corrected + closed; memory `phipo-is-species-neutral`. | applied — **the ruling is the curator's, the vocabulary claims are verified** against `phipo-base.obo` (release 2026-03-12). Open question for a future refresh: the label-hit counts are release-specific; re-check if PHIPO ever adds species-specific labels. |

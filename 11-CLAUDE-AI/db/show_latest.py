...
(see docs/MODULARITY-PLAN.md). Prefer `python3 -m phiweaver.tracking.show_latest`; this is kept so existing
...
for _parent in _here.parents:                 # add repo root when phiweaver isn't installed
    if (_parent / "AGENTS.md").exists():
        ...
        break
runpy.run_module("phiweaver.tracking.show_latest", run_name="__main__", alter_sys=True)

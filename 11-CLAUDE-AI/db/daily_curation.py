...
(see docs/MODULARITY-PLAN.md). Prefer `python3 -m phiweaver.tracking.daily_curation`; this is kept so existing
...
for _parent in _here.parents:                 # add repo root when phiweaver isn't installed
    if (_parent / "AGENTS.md").exists():
        ...
        break
runpy.run_module("phiweaver.tracking.daily_curation", run_name="__main__", alter_sys=True)

...
(see docs/MODULARITY-PLAN.md). Prefer `python3 -m phiweaver.pipeline.curation_pipeline`; this is kept so existing
...
for _parent in _here.parents:                 # add repo root when phiweaver isn't installed
    if (_parent / "AGENTS.md").exists():
        ...
        break
runpy.run_module("phiweaver.pipeline.curation_pipeline", run_name="__main__", alter_sys=True)
